...
def conv_block(in_channels,out_channels,kernels,padding=1,bias=False):
    layers = nn.Sequential(
        ...
        nn.Conv2d(in_channels,out_channels,kernels,padding=padding,bias=bias),
        nn.BatchNorm2d(out_channels),
        nn.ReLU(inplace=True),
        nn.Conv2d(out_channels,out_channels,kernels,padding=padding,bias=bias)
        )

    return layers


class Tomography_nCNN(nn.Module):
